...
This monorepo contains three main applications in the `./apps` directory:

- **Extension** → `apps/extension` - Web extension, ext for short
- **Mobile** → `apps/mobile` - React Native mobile app
- **Web** → `apps/web` - React Router framework mode web app

When referencing these apps in scopes, commit messages, or documentation, use the shortened form (ext, mobile, web).
